...
//This function is used to add tasks 
function addTaskToDOM(task){
    const li = document.createElement('li');

    /*Creating List element by which we add task name, 
    checkbox to know whether task is completed and delete task option*/
    li.innerHTML=`
    <div>
    <input type="checkbox" id="${task.id}" ${task.completed ? 'checked' : ''} class="custom-checkbox">
    <label for="${task.id}">${task.title}</label>
    </div>
    <button class="delete" data-id="${task.id}">Del</button>
    `;
    tasklist.append(li);
}
